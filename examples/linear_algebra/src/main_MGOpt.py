...
parser.add_argument('--interpolation', type=str, default='constant')
parser.add_argument('--μ', type=int, default=1)#3)
parser.add_argument('--ν', type=int, default=1)#3)
parser.add_argument('--n_cycles', type=int, default=2)
parser.add_argument('--n_epochs', type=int, default=100000)
parser.add_argument('--n_lays_enc', type=int, default=4)
parser.add_argument('--n_lays_dec', type=int, default=4)
parser.add_argument('--n_lvls', type=int, default=2)
...
import MGOpt # from MGOpt import V_cycle
...
# 6. Training
# 6.2 (Implement ...)
def train(_vars, train_data_loader, valid_data_loader, vocabulary_source, 
                                                       vocabulary_target):
  ...
  # Constants
  _vars.n_gradients_update = 1#10   # (for gradients accumulation)
  n_monitoring = _vars.n_monitoring = 500#1#500*50  # aka n_V_cycles before monitoring
  iter_valid_data_loader = iter(valid_data_loader)
  # _vars.iter_dl_tr = iter(train_data_loader)
  # _vars.dl_tr = train_data_loader

  # Initialise variables
  _vars.counter_gradients_update = 0
  counter_monitoring = 0
  _vars.correct_training = 0 
  _vars.total_training = 0
  losses_training = []
  losses_validation = []
  ...
  for model in _vars.models.values(): model.train()   # train

  for epoch in range(args.n_epochs):
    for batch in train_data_loader:
      _vars.batch, _vars.vocabulary_target = batch, vocabulary_target
      MGOpt.V_cycle(_vars)

      losses_temp_training.append(_vars.loss.item())
      counter_monitoring += 1
      if first_loss:
        text = f'first training loss {_vars.loss.item() : .4e}'
        print(colored(text, 'blue') if colour_imported else text)
        print()
        first_loss = False

      if _vars.debug: print(_vars.loss.item())

      if counter_monitoring % n_monitoring == 0:
        t0_monitoring = time.time()
        model = _vars.models[_vars.FINE]
        model.eval()  # evaluate 
        with torch.no_grad():
          # (I) training set
          losses_training.append(np.mean(losses_temp_training))
          accuracies_training.append(_vars.correct_training / _vars.total_training)
          _vars.correct_training, _vars.total_training = 0, 0

          text = f'training loss {_vars.loss.item() : .4e}'
          text += f'\ttraining accuracy {100 * accuracies_training[-1] : .4f}%'
          print(colored(text, 'blue') if colour_imported else text)

          # We'll take as an example the first entry in the batch
          print_example(_vars.predictions, _vars.sources, _vars.targets_output, 
                                  vocabulary_source, vocabulary_target, 'blue')
          #######################################
          ...
          t_monitor = time.time()
          print(f'Monitoring period: {t_monitor - t0_monitor}s')
        ...
        print(f'Time spent on monitoring: {time.time() - t0_monitoring}s')
        model.train()
...
def main(task):
  ...
  # Parameters
  dim_hidden = 256  # 8 for debugging (cpu)
  att_heads = 8
  num_enc_layers_0 = args.n_lays_enc
  num_dec_layers_0 = args.n_lays_dec
  dim_ff = 1024
  ...
  assert (num_enc_layers_0 % 2**(args.n_lvls-1) == 0) and \
         (num_dec_layers_0 % 2**(args.n_lvls-1) == 0)

  _vars = copy.copy(args)
  _vars.models = models = {}
  _vars.optimizers = optimizers = {}
  _vars.loss_function = loss_function

  for lvl in reversed(range(args.n_lvls)):
    num_enc_layers = num_enc_layers_0 // 2**(args.n_lvls - 1 - lvl)
    num_dec_layers = num_dec_layers_0 // 2**(args.n_lvls - 1 - lvl)
    print(f'Building model w/ {num_enc_layers} encoder layers' \
        + f' and {num_dec_layers} decoder layers')
    # Model
    model = Trans(
      num_enc_layers, num_dec_layers, dim_hidden, dim_ff, 
      att_heads, dropout, src_vocab, tgt_vocab, DEVICE
    )
    assert args.opt in 'Adam SGD'.split()
    optimizer = (Adam if args.opt=='Adam' else SGD)(model.parameters(), 
                                                      lr=learning_rate)

    models[lvl] = model
    optimizers[lvl] = optimizer

  if args.load is not None: 
    model = models[args.n_lvls-1]
    try:
  ...
  #     + f' and {num_dec_layers} decoder layers')
  # (models, optimizers, losses_tr, losses_va, accuracies_tr,
  #   accuracies_va) = 
  train(_vars, train_data_loader, valid_data_loader, src_vocab, tgt_vocab)
