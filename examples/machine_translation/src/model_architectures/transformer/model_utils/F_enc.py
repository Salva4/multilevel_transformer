...
class F_enc(nn.Module):
  def __init__(self, model_dimension, num_heads, dim_ff):
    ...
    self.phi1 = lambda x, mask_pad_src, self_attn, self_attn_layer_norm: \
      self_attn(
        x=self_attn_layer_norm(x), mask_pad=mask_pad_src, add_mask_attn=False,
      )
    self.phi2 = lambda x, mlp, mlp_layer_norm: mlp(mlp_layer_norm(x))

  def forward(self, x, mask_pad_src=None):
    phi1_x = self.phi1(
      x, mask_pad_src, self.self_attn, self.self_attn_layer_norm
    )
    phi2_x = self.phi2(x + phi1_x, self.mlp, self.mlp_layer_norm)
