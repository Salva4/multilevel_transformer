...
def train(model, optimizer, loss_function, train_data_loader, valid_data_loader,
  ...
  time_limit_minutes = 1e9   # place_value: 20, sort: 5*60, algebra: 1e9 (infty)
...
def main(task):
  ...
  colab_path = 'drive/MyDrive/Colab Notebooks/2 - USI DLL/data/'
...
############# POST-TREATMENT (algebra)
# Treatment for 7.5 (described in the report) - Run in cpu

'''
# It is necessary to import torch, numpy, matplotlib.pyplot and to define the 
#...function "plot_training" again:
## import torch, numpy as np, matplotlib.pyplot as plt
##def plot_training(...

# Load history of losses and accuracies
checkpoint = torch.load(
    'drive/MyDrive/Colab Notebooks/2 - USI DLL/models/state_ex8_alg1.pt',
    map_location = 'cpu')
losses_training = checkpoint['losses_training']
...
accuracies_validation = checkpoint['accuracies_validation']
for k in [1, 50, 200]:
  losses_training_ = [np.mean([
      losses_training[k*i+j] for j in range(k)
  ]) for i in range(int(len(losses_training)/k))]
  losses_validation_ = [np.mean([
      losses_validation[k*i+j] for j in range(k)
  ]) for i in range(int(len(losses_validation)/k))]
  accuracies_training_ = [np.mean([
      accuracies_training[k*i+j] for j in range(k)
  ]) for i in range(int(len(accuracies_training)/k))]
  accuracies_validation_ = [np.mean([
      accuracies_validation[k*i+j] for j in range(k)
  ]) for i in range(int(len(accuracies_validation)/k))]
  plot_training(losses_training_, losses_validation_, accuracies_training_, accuracies_validation_)
'''
######################################
