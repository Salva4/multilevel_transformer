  ...
  for j, fn in enumerate(fns):
    ...
    with open(f'../outputs/{dir_nm}/outputs/{fn}', 'r') as f:
      ...
      for line in f:
        ...
        elif line.startswith('val bleu'):
          bleu = float(re.findall('val bleu(.*)', line)[0])
          bleus.append(bleu)
    ...
    axs[0].plot(bleus, color=colors[j], label=f'{fn}', linestyle=linestyle)
    if lim is not None: axs[0].set_xlim(0, lim[0])
    axs[1].plot(cost, bleus, color=colors[j], label=f'{fn}', 
                                          linestyle=linestyle)
    ...
    dv = 1/2*j/len(fns)*2
    axs[0].vlines(changes+dv, 0+dv, 1, color=colors[j], linestyle=':')
    # try: axs[1].vlines([times[change]+dv for change in changes], 0+dv, 100+dv,
    #                                            color=colors[j], linestyle=':')
    # except: pass

    try: print(fn, f'max: {max(bleu)}')
    except: pass

  titles = ['bleu_val x 25000 batches (prop. to #epochs)', 
            'bleu_val vs. #gradient computations']
