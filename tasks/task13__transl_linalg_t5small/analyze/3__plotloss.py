...
def main(contains, fig=None, axs=None, lim=None):
  ...
  for j, fn in enumerate(fns):
    ...
    ## Extract losses
    losses_tr, losses_va, changes, times = [], [], [], []
    # predicted, correct = [], []
    # last = None
    with open(f'../outputs/{dir_nm}/outputs/{fn}', 'r') as f:
      ctr = 0
      for line in f:
        if re.findall('training loss(.*)training accuracy', line):
          loss = float(re.findall('training loss(.*)training accuracy', line)[0])
          losses_tr.append(loss)

        elif re.findall('validation loss(.*)validation accuracy', line):
          loss = float(re.findall('validation loss(.*)validation accuracy', line)[0])
          losses_va.append(loss)

        elif line.startswith('Training model w/'):
          changes.append(ctr)

        elif line.startswith('time monitor:'):
          time = re.findall('time monitor: (.*)s', line)[0]
          times.append(float(time))



        # elif last == 'validation':
        #   if re.findall('.*PREDICTED ANSWER:(.*)', line):
        #     line = line.replace(',', ' ,').replace('.', ' .').replace('<eos>', ' <eos>')
        #     predicted.append(re.findall('.*PREDICTED ANSWER:(.*)', line)[0].strip().split())

        #   if re.findall('.*CORRECT ANSWER:(.*)', line):
        #     line = line.replace(',', ' ,').replace('.', ' .').replace('<eos>', ' <eos>')
        #     correct.append([re.findall('.*CORRECT ANSWER:(.*)', line)[0].strip().split()])



    # bleus = []
    # for pred, corr in zip(predicted, correct):
    #   bleus.append(bleu_score(pred, corr))
    ...
    axs[0].plot(losses_va, color=colors[j], label=f'{fn}', linestyle=linestyle)
    if lim is not None: axs[0].set_xlim(0, lim[0])
    axs[1].plot(cost, losses_va, color=colors[j], label=f'{fn}', 
                                          linestyle=linestyle)
    ...
    try: print(fn, f'max: {max(losses_va)}')
    except: pass
